...
<div class="product_wrap bg-white overflow-hidden group relative border border-gray-300 hover:shadow-lg transition-all duration-300">
 ...
 <img src="{{ $product->primary_image }}" alt="{{ $product->name }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110">
                 ...
                 <div class="product_action_box absolute bottom-4 left-0 right-0 flex justify-center space-x-2 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
                                 <form x-data="{}" action="{{ route('cart.add') }}" method="POST" @submit.prevent="fetch($el.action, {
                                     method: 'POST',
            ...
            .then(data => {
                if (data && data.success) {
                    // console.log('Dispatching open-cart with HTML length:', data.html.length);
                    // alert('HTML Length: ' + data.html.length); 
            ...
            .catch(e => {
             ...
             <button @click.prevent="$dispatch('open-quick-view', { id: {{ $product->id }} })" class="h-10 w-10 bg-white text-gray-800 hover:bg-primary-500 hover:text-white rounded-full shadow-md flex items-center justify-center transition-colors" title="Quick View"> <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
 </button>
 <button @click.prevent="fetch('{{ route('wishlist.toggle', $product->id) }}', {
 method: 'POST',
 headers: {
 ...
 alert('Something went wrong with the wishlist.');
 })" class="h-10 w-10 bg-white text-gray-800 hover:bg-primary-500 hover:text-white rounded-full shadow-md flex items-center justify-center transition-colors" title="Wishlist">
