<x-app-layout>
 ...
 {{ $type === 'billing' ? __('Create Billing Address') : __('Create Shipping Address') }}
 </h3>

 <form action="{{ route('addresses.store') }}" method="POST" x-data="{ type: '{{ old('address_type', $type) }}' }">
 @csrf
 ...
 <input type="text" name="alias" value="{{ old('alias') }}" placeholder="My Home" class="mt-1 block w-full rounded-md border-gray-300 text-gray-900 shadow-md focus:border-indigo-500 focus:ring-indigo-500">
 ...
 <input type="text" name="city" value="{{ old('city') }}" class="mt-1 block w-full rounded-md border-gray-300 text-gray-900 shadow-md focus:border-indigo-500 focus:ring-indigo-500">
 ...
 <a href="{{ $type === 'billing' ? route('addresses.billing') : route('addresses.shipping') }}" class="text-gray-600 hover:text-gray-900 text-sm font-medium">{{ __('Cancel') }}</a>
