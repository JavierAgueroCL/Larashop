                    ...
                    <div class="bg-white overflow-hidden shadow-md sm:rounded-lg border border-gray-300">
                        <div class="p-6 text-gray-900">
                            <div class="flex justify-between items-center mb-6">
                                <h3 class="text-lg font-bold text-gray-800">{{ __("My Shipping Addresses") }}</h3>
                            ...
                            @if($addresses->isEmpty())
                                <p class="text-gray-500 italic border border-dashed border-gray-300 rounded-lg p-6 text-center bg-gray-50">{{ __("You haven't saved any shipping addresses yet.") }}</p>
                            @else
                                    ...
                                    @foreach($addresses as $address)
                                        <div class="border border-gray-300 bg-white rounded-lg p-4 relative hover:shadow-lg transition-all group">
                                            <div class="absolute top-4 right-4 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <a href="{{ route('addresses.edit', $address) }}" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">Edit</a>
                                                <form action="{{ route('addresses.destroy', $address) }}" method="POST" onsubmit="return confirm('{{ __('Are you sure you want to delete this address?') }}');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-red-600 hover:text-red-800 text-sm font-medium ml-2">Delete</button>
                                                </form>
                                            </div>
                                            
                                            <div class="mb-2">
                                                <span class="font-bold text-lg text-gray-800">{{ $address->alias ?? 'Address' }}</span>
